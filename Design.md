...
For a one-off build, the rule set can just live inline in your response as the stated basis for the generated output. For scoring a build against a specific brand's _catalog_ design system (as opposed to a one-off supplied reference), that is a different job, handled by DesignMD's `certify_conformance` tool where available, not by this section.
