...
if (!file || !fs.existsSync(file)) {
 console.error('embed-prompt: image file required'); process.exit(1); 
}
...
prompt = readJpegCom(buf);
 ...
 prompt = JSON.parse(fs.readFileSync(sidecar, 'utf8')).prompt ?? null; 
  ...
  if (prompt == null) {
 console.error('embed-prompt: no embedded prompt found'); process.exit(2); 
}
...
if (!prompt) {
 console.error('embed-prompt: --prompt or --prompt-file required'); process.exit(1); 
}
...
if (isPng) {
  ...
  if (iend < 8) {
 console.error('embed-prompt: malformed PNG'); process.exit(1); 
}
...
parts.push(chunk);
 ...
 console.error('embed-prompt: prompt too long for a JPEG segment'); process.exit(1); 
}
